...
- \`component: "condition"\` 只支持两种互斥模式，不能混用字段：
  - 布尔模式必须使用 \`when\` 和 \`then\` 数组；可选 \`else\` 或 \`default\`。\`when\` 可写布尔值、纯表达式或契约允许的 \`{ "path": "/field" }\` 绑定。
  - 多值模式必须使用 \`match\` 和 \`cases\` 对象；可选 \`default\`。\`cases\` 的键必须与 \`match\` 求值后转成的字符串完全一致。
- 不得使用不存在的 \`condition\` 属性，也不得在 \`condition\` 组件上写 \`children\`。分支组件只能通过 \`then\`、\`else\`、\`cases\` 或 \`default\` 引用，且同一组件 ID 只能属于一个分支。
- \`when\` 和 \`match\` 可以使用契约允许的路径绑定；不要把 JSON Pointer 嵌入表达式字符串。动态表达式使用 \`"\${$root.field}"\` 形式。
